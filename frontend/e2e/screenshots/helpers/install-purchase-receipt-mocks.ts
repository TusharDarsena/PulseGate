...
const TRANSACTION_HASH = '8'.repeat(64);
...
/**
 * Supplies a completed owner-scoped receipt without reproducing the financial
 * transaction. The page still renders its normal authenticated receipt UI.
 */
export async function installPurchaseReceiptConfirmedMocks(page: Page): Promise<void> {
  ...
  if (!supabaseUrl || !ticketContractId) {
    throw new Error('VITE_SUPABASE_URL and VITE_TICKET_CONTRACT_ID are required for receipt capture.');
  }
  ...
  await page.route('**/rest/v1/published_events**', async (route) => {
    ...
    if (!requested?.includes(EVENT_ID)) {
      throw new Error(`Unexpected receipt event lookup: ${requested ?? 'missing event_id'}`);
    }
  ...
  await page.route('**/functions/v1/purchase-operation', async (route) => {
    ...
    if (body?.action === 'get' && body.operationId === OPERATION_ID) {
      await fulfillJson(route, { operation, attempt: null });
      return;
    }
    throw new Error(
      `Unexpected purchase-operation request during receipt capture: ${body?.action ?? 'missing'}`,
    );
  });
